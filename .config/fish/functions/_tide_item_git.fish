function _tide_item_git
    # add yadm status to homedir
    if string match -q "$HOME" (pwd)
        set yadm_repo --git-dir $HOME/.local/share/yadm/repo.git/
    end

    if git $yadm_repo branch --show-current 2>/dev/null | string replace -r ".+(.{$tide_git_truncation_length})" '…$1' | read -l location
        git $yadm_repo rev-parse --git-dir --is-inside-git-dir | read -fL gdir in_gdir
    ...
    stat=(git $yadm_repo $_set_dir_opt --no-optional-locks status --porcelain 2>/dev/null) \
        string match -qr '(0|(?<stash>.*))\n(0|(?<conflicted>.*))\n(0|(?<staged>.*))
(0|(?<dirty>.*))\n(0|(?<untracked>.*))(\n(0|(?<behind>.*))\t(0|(?<ahead>.*)))?' \
        "$(git $yadm_repo $_set_dir_opt stash list 2>/dev/null | count
        string match -r ^UU $stat | count
        string match -r ^[ADMR]. $stat | count
        string match -r ^.[ADMR] $stat | count
        string match -r '^\?\?' $stat | count
        git $yadm_repo rev-list --count --left-right @{upstream}...HEAD 2>/dev/null)"
